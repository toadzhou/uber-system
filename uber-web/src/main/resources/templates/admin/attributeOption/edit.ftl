...
<div class="pageContent">
	<form action="${base}/admin/attributeOption/update" method="post" class="pageForm required-validate" onsubmit="return validateCallback(this, dialogAjaxDone);">
		<div class="pageFormContent" layoutH="58">
            <input type="hidden" name="id" value="${bean.id}"/>
    		<p>
                <label>选项值：</label>
                <input type="text" name="optionName" value="${bean.optionName}" placeholder="选项值" size="20" />
            </p>
    		<p>
                <label>关联属性Id：</label>
                <input type="text" name="attributeId" value="${bean.attributeId}" placeholder="关联属性Id" size="20" />
            </p>
    		<p>
                <label>创建人：</label>
