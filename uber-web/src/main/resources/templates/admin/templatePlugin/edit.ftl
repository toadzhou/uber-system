...
<div class="pageContent">
	<form action="${base}/admin/templatePlugin/update" method="post" class="pageForm required-validate" onsubmit="return validateCallback(this, dialogAjaxDone);">
		<div class="pageFormContent" layoutH="58">
            <input type="hidden" name="id" value="${bean.id}"/>
    		<p>
                <label>模版Id：</label>
                <input type="text" name="templateId" value="${bean.templateId}" placeholder="模版Id" size="20" />
            </p>
    		<p>
                <label>插件Id：</label>
                <input type="text" name="pluginId" value="${bean.pluginId}" placeholder="插件Id" size="20" />
            </p>
    		<p>
                <label>创建人：</label>
